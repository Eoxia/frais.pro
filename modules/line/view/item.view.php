...
<div class="table-row line" data-id="<?php echo esc_attr( $line->id ); ?>" >
	...
	<div class="table-cell line-content wpeo-form">
		<div class="date form-element group-date">
			...
			<input type="text" class="mysql-date" style="width: 0px; padding: 0px; border: none;" name="date" value="<?php echo esc_attr( $line->date['raw'] ); ?>" <?php echo ( $note_is_closed ? 'readonly="readonly"' : '' ); ?>/>
		...
		<div class="libelle form-element <?php echo esc_attr( ! empty( $line->line_status ) && ( false === $line->line_status['status'] ) && in_array( 'title', $line->line_status['errors'], true ) ? 'input-error' : '' ); ?>">
			...
			<input type="text" name="title" placeholder="<?php esc_attr_e( 'Label', 'frais-pro' ); ?>" value="<?php echo esc_html( $line->title ); ?>" <?php echo ( $note_is_closed ? 'readonly="readonly"' : '' ); ?>/>
		</div>
		...
		<div class="km form-element">
			...
			<input type="text" name="distance" value="<?php echo esc_html( $line->distance ); ?>" <?php echo ( $note_is_closed || ( ! empty( $line->taxonomy ) && ! empty( $line->taxonomy[ Line_Type_Class::g()->get_type() ] ) && ! empty( $line->taxonomy[ Line_Type_Class::g()->get_type() ][0] ) && empty( $line->taxonomy[ Line_Type_Class::g()->get_type() ][0]->special_treatment ) ) ? 'readonly="readonly"' : '' ); ?>/>
		</div>
		<div class="ttc form-element <?php echo esc_attr( ! empty( $line->line_status ) && ( false === $line->line_status['status'] ) && in_array( 'amount', $line->line_status['errors'], true ) ? 'input-error' : '' ); ?>">
			...
			<input type="text" name="tax_inclusive_amount" value="<?php echo esc_html( $line->tax_inclusive_amount ); ?>" <?php echo ( $note_is_closed ? 'readonly="readonly"' : '' ); ?>/>
		</div>
		<div class="tva form-element <?php echo esc_attr( ! empty( $line->line_status ) && ( false === $line->line_status['status'] ) && in_array( 'amount', $line->line_status['errors'], true ) ? 'input-error' : '' ); ?>">
			...
			<input type="text" name="tax_amount" value="<?php echo esc_html( $line->tax_amount ); ?>" <?php echo ( $note_is_closed ? 'readonly="readonly"' : '' ); ?> />
		</div>
