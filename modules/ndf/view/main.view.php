...
<section class="eox-note-frais">
	...
	<?php if ( ! empty( $status ) && is_array( $status ) && in_array( 'publish', $status, true ) ) : ?>
		<div class="add-ndf wpeo-button button-blue action-attribute"
				data-action="create_ndf"
				data-nonce="<?php echo esc_attr( wp_create_nonce( 'create_ndf' ) ); ?>"><i class="button-icon fa fa-plus-circle"></i><span><?php esc_html_e( 'Add' ); ?></span></div>
	<?php endif; ?>

	<div class="main-container">
		<div class="liste-note wpeo-grid grid-3">
			<?php
			if ( ! empty( $ndfs ) ) :
				foreach ( $ndfs as $ndf ) :
