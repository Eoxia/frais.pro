...
window.eoxiaJS.noteDeFrais.NDFL.init = function() {
	...
	jQuery( document ).on( 'click', '.photo .dashicons-image-rotate-right', function( event ) {
		event.preventDefault();
		var currentRotation = jQuery( this ).attr( 'data-rotation' );
		currentRotation = parseInt( currentRotation ) + 90;

		jQuery( this ).closest( '.photo' ).find( 'img' ).css( {
			'-webkit-transform': 'rotate(' + currentRotation + 'deg)',
			'-moz-transform': 'rotate(' + currentRotation + 'deg)',
			'transform': 'rotate(' + currentRotation + 'deg)'
		});
		jQuery( this ).attr( 'data-rotation', currentRotation );
	} );
